Design or refine an experiment-only portable skill for the task: Bioinformatics Database Query.

Task goal:
Answer gene, protein, variant, and disease lookup tasks against public APIs and local lookup tables with normalized identifiers.

Constraints:
- Only create or modify files under `experiments/sc_skills/database-query`.
