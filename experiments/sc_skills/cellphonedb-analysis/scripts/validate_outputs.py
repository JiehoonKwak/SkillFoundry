...
def validate_tsv(path: Path, required_columns: list[str]) -> None:
    with path.open("r", encoding="utf-8", newline="") as handle:
        ...
        if first_row is None:
            raise AssertionError(f"Empty TSV file: {path}")
...
def validate_json(path: Path, required_keys: list[str]) -> None:
    ...
    if not payload.get("written_files"):
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate the portable CellPhoneDB starter outputs.")
