Design or refine an experiment-only portable skill for the task: Sequence Analysis and Primer Design.

Task goal:
Handle sequence identification, feature lookup, variant inspection, and primer-oriented follow-up tasks in one portable workflow.

Constraints:
- Only create or modify files under `experiments/sc_skills/sequence-analysis`.
