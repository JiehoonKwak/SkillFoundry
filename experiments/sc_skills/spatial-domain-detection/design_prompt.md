Design or refine an experiment-only portable skill for the task: Spatial Domain Detection.

Task goal:
Detect spatial regions or domains with graph-based models and explain the evidence behind the chosen partition.

Constraints:
- Only create or modify files under `experiments/sc_skills/spatial-domain-detection`.
